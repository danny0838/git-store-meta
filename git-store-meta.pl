...
my $GIT_STORE_META_VERSION   = "1.2.5";
...
sub install_hooks {
    ...
    my $pre_commit_options = "";
    if ($argv{'directory'}) {
        $pre_commit_options = "-d";
    }
...
\$(dirname "\$0")/git-store-meta.pl --update $pre_commit_options ||
\$(dirname "\$0")/git-store-meta.pl --store $pre_commit_options ||
exit 1
