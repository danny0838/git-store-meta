...
my $GIT_STORE_META_VERSION   = "1.1.1";
...
sub apply {
    ...
    # 1.1.* and 1.0.* can share same apply procedure
    # (bad file name are never added in 1.0.*)
    if ($version =~ m!^1\.1\..+$! || $version =~ m!^1\.0\..+$!) {
        my $count = 0;
